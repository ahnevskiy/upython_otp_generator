from time import sleep_ms

from app.utils.str_helper import str_rjust


class LCD1602:
    def __init__(self, i2c, addr=0):
        self.enabled = True
        self.i2c = i2c
        self.buf = bytearray(1)
        self.BK, self.RS, self.E = 0x08, 0x00, 0x04
        self.ADDR = addr if addr else self.auto_addr()
        try:
            self.set_cmd(0x33)
        except Exception as e:
            print("Cant initialize a LCD1602: ({e})".format(e=e))
            self.set_enable(False)
            return
        
        self.set_cmd(0x33)
        sleep_ms(5)
        self.send(0x30)
        sleep_ms(5)
        self.send(0x20)
        sleep_ms(5)
        for i in [0x28, 0x0C, 0x06, 0x01]:
            self.set_cmd(i)
        self.px, self.py = 0, 0
        self.pb = bytearray(' '*16, 'utf-8')
        self.version = '2.0'
        self.clear()

    def set_enable(self, value=True):
        self.enabled = value
        
    def set_reg(self, dat):
        if self.enabled:
            self.buf[0] = dat
            self.i2c.writeto(self.ADDR, self.buf)
            sleep_ms(1)

    def send(self, dat):
        if self.enabled:
            d = (dat & 0xF0) | self.BK | self.RS
            self.set_reg(d)
            self.set_reg(d | 0x04)
            self.set_reg(d)

    def set_cmd(self, cmd):
        if self.enabled:
            self.RS = 0
            self.send(cmd)
            self.send(cmd << 4)

    def set_dat(self, dat):
        if self.enabled:
            self.RS = 1
            self.send(dat)
            self.send(dat << 4)

    def auto_addr(self):
        if self.enabled:
            for i in range(32, 63):
                try:
                    if self.i2c.readfrom(i, 1):
                        return i
                except:
                    pass
            raise Exception('I2C address detect error!')

    def clear(self):
        if self.enabled:
            self.set_cmd(1)

    def backlight(self, on):
        if self.enabled:
            if on:
                self.BK=0x08
            else:
                self.BK=0
            self.set_cmd(0)

    def on(self):
        if self.enabled:
            self.set_cmd(0x0C)

    def off(self):
        if self.enabled:
            self.set_cmd(0x08)

    def shl(self): 
        if self.enabled:
            self.set_cmd(0x18)

    def shr(self):
        if self.enabled:
            self.set_cmd(0x1C)

    def char(self, ch, x=-1, y=0):
        if self.enabled:
            if x >= 0:
                a = 0x80
                if y > 0:
                    a = 0xC0
                self.set_cmd(a + x)
            self.set_dat(ch)

    def puts(self, s, x=0, y=0):
        if self.enabled:
            if type(s) is not str:
                s = str(s)
            if len(s) > 0:
                self.char(ord(s[0]), x, y)
                for i in range(1, len(s)):
                    self.char(ord(s[i]))

    def newline(self):
        if self.enabled:
            self.px = 0
            if self.py < 1:
                self.py += 1
            else:
                for i in range(16):
                    self.char(self.pb[i], i)
                    self.char(32, i, 1)
                    self.pb[i] = 32

    def print(self, s):
        if self.enabled:
            if type(s) is not str:
                s = str(s)
            for i in range(len(s)):
                d = ord(s[i])
                if d == ord('\n'):
                    self.newline()
                else:
                    self.char(d, self.px, self.py)
                    if self.py:
                        self.pb[self.px] = d
                    self.px += 1
                    if self.px > 15:
                        self.newline()

    def print_in_line(self, string, line_number):
        if self.enabled:
            self.puts(str_rjust(string, " ", 16), 0, line_number)
